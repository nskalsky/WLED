...
void parseMetarIntoFields(const String& metar) {
  ...
  client.setInsecure();
  // (Optional) smaller TLS buffers can reduce peak RAM/stack pressure:
  ...
  // (Optional) HTTP/1.0 disables chunked transfer, sometimes helps:
  // http.useHTTP10(true);
  ...
  // --------- key change: stream parse to avoid big stack/String copies ----------
  // Use heap-backed JSON (DynamicJsonDocument) and parse directly from stream.
  WiFiClient& stream = http.getStream();
  DynamicJsonDocument doc(2048); // plenty for a single-station METAR JSON
...
public:
  // ---------- core hooks ----------
  void setup() override {
    SA_DBG("setup: airport=%s, interval=%u min, enabled=%d, autoBoot=%d\n",
...
void loop() override {
    ...
    const uint32_t intervalMs = (uint32_t)max<uint16_t>(1, updateInterval) * 60000UL;
    const unsigned long now = millis();
    const bool due = enabled && ((now - lastRunMs) >= intervalMs);

    // Run only if interval elapsed (enabled) or a manual refresh was requested
    if (!due && !refreshNow) return;

    SA_DBG("loop: %s (%lums since last)\n", refreshNow ? "manual" : "interval", (now - lastRunMs));
    busy = true;
    bool ok = fetchMetarOnce();
    lastRunMs = millis();
    busy = false;
    refreshNow = false;
    ...
    if (!ok && successCount > 0) {
      ...
      SA_DBG("loop: fetch failed, set UNKNOWN color\n");
    }
  }
